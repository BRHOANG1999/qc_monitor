...
# Sheets I/O lives in src/utils/sheets.py now. These re-exports keep
# the existing callers in tabs/data_log_xref, tabs/maintenance,
# tabs/cage_index, utils/assignments, notifications/surgery_digest
# working unchanged during the transition. A follow-up PR can rewire
# them to import directly from src.utils.sheets and drop this block.
from src.utils.sheets import (  # noqa: F401  (re-exported for callers)
    _SHEETS_API_SCOPES, _cache, _cache_lock, _dedupe,
    _FETCH_TIMEOUT_SEC, _find_column, _last_fetched, _load_sheet,
    _load_sheet_via_api, _norm_colname, _normalize_text, _resolve_sa_path,
    _safe_date, _sa_service_cache, _sa_service_lock, _sheets_api,
    cache_has_entry,
)
